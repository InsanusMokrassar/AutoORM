...
class DatabaseManager(config : com.github.insanusmokrassar.iobjectk.interfaces.IObject<Any>) {
    ...
    private val databaseDrivers: MutableMap<String, com.github.insanusmokrassar.AbstractDatabaseORM.core.drivers.databases.interfaces.DatabaseDriver> = HashMap()
    ...
    private fun getDatabaseDriver(name: String) : com.github.insanusmokrassar.AbstractDatabaseORM.core.drivers.databases.interfaces.DatabaseDriver {
        if (databaseDrivers.containsKey(name)) {
            ...
            val driver = (Class.forName(config.get("classpath")).kotlin.constructors.getFirst {
                it.parameters.size == 1 && (it.parameters[0].type.classifier as kotlin.reflect.KClass<*>).isSuperclassOf(parameters::class)
                    ...
                    as? com.github.insanusmokrassar.AbstractDatabaseORM.core.drivers.databases.interfaces.DatabaseDriver ?: throw IllegalStateException("Founded driver for name $name is not DatabaseDriver")
            if (config.get("cache")) {
